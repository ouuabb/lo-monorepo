...
```
[[res_xxx]]          按 RID 精确链接（唯一身份）
[[res_xxx|显示别名]]  按 RID 链接，渲染时显示别名（alias 为空时显示资源 name）
```

> Wikilink 的 identity 是 **rid**（永不变化）；`[[名字]]` 旧语法已移除，不参与身份解析。
> 改名只改 `resource.name`，不重写 `[[rid]]` 引用——关系与显示名自动跟随。
...
|------|------|-------------------|
| 标题 | # 后的文字，可随时修改 | 不参与 wikilink 身份；仅作显示名（alias ?? name）来源 |
| 路径 | 文件在磁盘的位置 | 不直接参与 wikilink，但 sync 重命名检测 |
| RID | 资源的永久 ID，永不改变 | **Wikilink 唯一身份**：`[[rid]]` 直接指向，改名/移动均不受影响 |
