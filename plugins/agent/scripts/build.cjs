...
/** 打包进插件的顶层条目（相对插件目录） */
const INCLUDE_ENTRIES = ['plugin.json', 'src', 'extension', 'ui', 'package.json'];
...
async function buildAll(options = {}) {
  ...
  for (const name of plugins) {
    if (pluginId && name !== pluginId) continue;
